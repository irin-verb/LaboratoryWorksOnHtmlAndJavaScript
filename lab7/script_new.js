...
var funcs = {
	f1: {
		func: x => x * (Math.sqrt(Math.pow(Math.sin(x + 10), 3))) + (Math.pow(x, 3) - Math.cos(x)) / x,
	},
	f2: {
		func: x => Math.pow(Math.sin(x), 2) - Math.abs(Math.sin(x - 4)),
	},
	f3: {
		func: x => Math.exp(x - 2) + Math.pow(x, 3) + 2 * x * Math.log(x + 3) / 7,
	},
};

Object.keys(funcs).forEach(funcName => {
	let funcObj = funcs[funcName];
	Object.keys(variants).forEach(variant => funcObj[variant] = variants[variant](funcObj.func));
});


function calculate() {
	var func = funcs[document.querySelector('input[name="func_types"]:checked').value].func;
	var chars = Array.from(document.querySelectorAll('input[name="calc_chars"]:checked'), element => element.value);
	var vars = Array.from(document.querySelectorAll('input[name="func_variants"]:checked'), element => element.value);

	var min = document.getElementById('a').value;
	var max = document.getElementById('b').value;
	var step = document.getElementById('h').value;

	if (argumentParamsAreValid(min, max, step)) {
		min = parseFloat(min);
		max = parseFloat(max);
		step = parseFloat(step);
		
		var result = calculateChars(func, chars, vars, min, max, step);
		showResult(chars, result);
	}
}
...
function calculateChars(func, chars, vars, min, max, step) {
	...
	result = chars.map(char => char(x, y));

	chars.map(char => {
		if char in Object.keys(func)
	});
	return result;
}
